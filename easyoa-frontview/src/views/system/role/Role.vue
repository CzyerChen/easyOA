...
export default {
  ...
  computed: {
    columns () {
      ...
      return [{
        ...
        sorter: true,
        sortOrder: sortedInfo.field === 'createTime' && sortedInfo.order
      }, {
        title: '修改时间',
        dataIndex: 'updateTime',
        sorter: true,
        sortOrder: sortedInfo.field === 'updateTime' && sortedInfo.order
      }, {
        title: '操作',
        dataIndex: 'operation',
  ...
  methods: {
    ...
    search () {
      ...
      // 获取当前列的排序和列的过滤规则
      if (sortedInfo) {
        sortField = sortedInfo.field === 'undefined'?"createTime":sortedInfo.field
        sortOrder = sortedInfo.order === 'undefined'?"DESC":sortedInfo.order
      }
      this.fetch({
        sortField: sortField,
        sortOrder: sortOrder,
