  ...
  >
    <a-form :form="form">
      ...
      <a-form-item label="密码" v-bind="formItemLayout">
        <a-tooltip title="新用户默认密码为 1234qwer,请尽快修改">
      ...
      <!-- <a-form-item label='工号'
        ...
        <a-input v-model="user.userCode"
                 v-decorator="['userCode',{rules:[{required:true,message:'工号不能为空'}]}]"/>
      </a-form-item>-->
      ...
      <a-form-item v-hasPermission="'data:super'" label="所属公司" v-bind="formItemLayout">
        <a-select
          mode="default"
    ...
    handleUserNameBlur() {
      ...
      if (username.length) {
        if (username.length > 50) {
          ...
          this.help = "用户名不能超过50个字符";
        } else if (username.length < 2) {
          this.validateStatus = "error";
          this.help = "用户名不能少于2个字符";
        } else if (username.charAt("@") === null) {
          this.validateStatus = "error";
