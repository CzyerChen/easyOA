  ...
  <a-card :bordered="false" class="card-area">
    ...
    <div>
      <div class="operator">
        ...
        <a-button type="primary" @click="downloadTemplate" style="margin-right: .5rem">员工模板下载</a-button>
        <!-- <a-button type="primary" @click="downloadTemplate1" style="margin-right: .5rem">
          汇报模板下载
        </a-button>-->
        <a-dropdown v-hasAnyPermission="'user:reset','user:export'">
          <a-menu slot="overlay">
            ...
            >密码重置</a-menu-item>
             <a-menu-divider />
            <a-menu-item v-hasPermission="'user:upload'" key="excel-user">
              <a-row :gutter="24">
                <a-col :span="12">
                  <a-upload
                    class="upload-area"
                ...
                <a-col :span="12">
                  <a-button
                    ...
                    :loading="uploading"
                  >{{uploading ? '上传中' : '点击上传' }}</a-button>
                </a-col>
              </a-row>
            </a-menu-item>
            <!-- <a-menu-item v-hasPermission="'user:upload'" key="excel-reporter">
              <a-upload
                class="upload-area"
                :fileList="fileList1"
                :remove="handleRemove1"
                :disabled="fileList1.length === 1"
...
export default {
  ...
  methods: {
    handleUpload() {
      ...
      formData.append("excel-user", fileList[0]);
      // debugger;
      this.uploading = true;
      this.$upload("user/upload/detail", formData)
        .then(r => {
          let data = r.data;
          if (data.code === 200) {
            if(data.data){
    ...
    handleUpload1() {
      ...
      const formData = new FormData();
      formData.append("excel-reporter", fileList1[0]);
      this.uploading = true;
      this.$upload("user/excel/reporter", formData)
        .then(r => {
          let data = r.data;
          if (data.code === 200) {
            this.$message.success("文件上传成功");
            this.importData1 = data.data;
            this.errors = data.error;
            this.times = data.time / 1000;
            this.uploading1 = false;
            this.importResultVisible1 = true;
            this.form.uploadFile = data.data;
          }
        })
        .catch(r => {
    ...
    downloadTemplate() {
      this.$download("user/template", {}, "员工信息导入模板.xlsx");
    },
    downloadTemplate1() {
      this.$download("user/reporter/template", {}, "员工汇报关系导入模板.xlsx");
    },
    ...
    resetPassword() {
      ...
      this.$confirm({
        title: "确定重置选中用户密码?",
        content: "当您点击确定按钮后，用户的密码将会重置为123qwe",
        centered: true,
        onOk() {
          let usernames = [];
          for (let key of that.selectedRowKeys) {
