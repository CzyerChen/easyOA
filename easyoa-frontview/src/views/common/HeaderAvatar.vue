<template>
  <div class="user-wrapper" >
    <span class="action">
      <a class="logout_title" title="暂不对外开放" target="_blank" >
        <a-icon type="question-circle-o"></a-icon>
      </a>
    </span>
    <header-notice class="action" style="padding-right:30px"/>
...
<script>
  ...
  export default {
    name: 'HeaderAvatar',
    components: {UpdatePassword,HeaderNotice},
    data() {
      return {
    ...
    methods: {
      ...
      logout() {
        this.$get(`logout/${this.user.userId}`).then(() => {
