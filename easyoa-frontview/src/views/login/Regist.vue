<template>
  <div class="user-layout-register">
    <a-form ref="formRegister" :autoFormCreate="(form)=>{this.form = form}" id="formRegister" style="padding: 15px">
      ...
      <a-form-item
        ...
        <a-input size="large" type="text" v-model="username" placeholder="邮箱" style="font-size:14px"></a-input>
      </a-form-item>
      <!--<a-form-item
        fieldDecoratorId="userCode"
        :fieldDecoratorOptions="{rules: [{ required: true, message: '请输入员工工号'}, { validator: this.handleUserCodeCheck }], validateTrigger: ['change', 'blur']}" >
        <a-input size="large" type="text" v-model="userCode" placeholder="工号" style="font-size:14px"></a-input>
      </a-form-item>-->
      ...
      <a-form-item
        ...
        <a-input size="large" type="password" autocomplete="false" placeholder="确认密码" style="font-size:14px"></a-input>
      </a-form-item>
      <!--
      <a-form-item
...
export default {
  ...
  methods: {
    ...
    handleUsernameCheck (rule, value, callback) {
      ...
      if (username.length) {
        if (username.length > 30) {
          ...
          this.$get(`user/check/name/${username}`).then((r) => {
            if (r.data) {
              callback()
            } else {
    ...
    handleNickNameCheck (rule, value, callback) {
      let nickName = this.nickName.trim()
      if (nickName.length) {
        if (nickName.length > 30) {
          callback(new Error('昵称不能超过30个字符'))
        } else if (nickName.length < 2) {
          callback(new Error('昵称不能少于2个字符'))
        }
      } else {
        callback()
      }
    },
/*    handleUserCodeCheck (rule, value, callback) {
      let userCode = this.userCode.trim()
      if (userCode.length === 0) {
        callback(new Error('工号不能为空'))
      } else if (userCode.length > 0) {
        this.$get(`user/check/code/${userCode}`).then((r) => {
          if (r.data) {
            callback()
          } else {
            this.validateStatus = 'error'
            callback(new Error('抱歉，该工号已被注册，请通过工号找回密码'))
          }
        })
      }
    },*/
